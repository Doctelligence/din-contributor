...
export const TABLE_NAME = 'T' + CONTRACT_ADDRESS.slice(2) + '_' + TABLE_VERSIONS;

export const POSTGRES_URL="postgres://neondb_owner:0zmWUvu6Txpa@ep-summer-surf-a53fy2kl-pooler.us-east-2.aws.neon.tech/neondb?sslmode=require"
const POSTGRES_URL_NON_POOLING="postgres://neondb_owner:0zmWUvu6Txpa@ep-summer-surf-a53fy2kl.us-east-2.aws.neon.tech/neondb?sslmode=require"
...
const POSTGRES_URL_NO_SSL="postgres://neondb_owner:0zmWUvu6Txpa@ep-summer-surf-a53fy2kl-pooler.us-east-2.aws.neon.tech/neondb"
const POSTGRES_PRISMA_URL="postgres://neondb_owner:0zmWUvu6Txpa@ep-summer-surf-a53fy2kl-pooler.us-east-2.aws.neon.tech/neondb?pgbouncer=true&connect_timeout=15&sslmode=require"
...
export async function update() {
  const sql = neon(POSTGRES_URL as string);
  await sql(`UPDATE ${TABLE_NAME} SET contributors = $1 WHERE project = $2`, ["hello world", 1]);
}
...
export async function appendValidators(project: number, validator: string) {
  const sql = neon(POSTGRES_URL as string);
  await sql(`UPDATE ${TABLE_NAME} SET validators = validators || $1 WHERE project = $2`, [";" + validator, project]);
}

export async function appendContributors(project: number, contributor: string) {
  const sql = neon(POSTGRES_URL as string);
  await sql(`UPDATE ${TABLE_NAME} SET contributors = contributors || $1 WHERE project = $2`, [";" + contributor, project]);
}

export async function clearContributors(project: number) {
  const sql = neon(POSTGRES_URL as string);
  await sql(`UPDATE ${TABLE_NAME} SET contributors = '' WHERE project = $1`, [project]);
}

export async function clearValidators(project: number) {
  const sql = neon(POSTGRES_URL as string);
  await sql(`UPDATE ${TABLE_NAME} SET validators = '' WHERE project = $1`, [project]);
}

// Get all ${TABLE_NAME} associated with the owner "0x1"
export async function getProjects(owner: string) {
  const sql = neon(POSTGRES_URL as string);
  const response = await sql`SELECT * FROM ${TABLE_NAME} WHERE owner = ${owner}`;
  return response;
}
...
async function removeContributors(project: number, address: string) {
  const sql = neon(POSTGRES_URL as string);
  await sql(`UPDATE ${TABLE_NAME} SET validators = REPLACE(REPLACE(REPLACE(validators, $1, ''), ';;', ';'), ';;', ';') WHERE project = $2`, [address, project]);
}
